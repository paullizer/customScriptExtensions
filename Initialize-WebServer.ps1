...
# Map ASE Upload Folder to s drive 
    try {
        New-PSDrive -Name "s" -Root ("\\" + $hostFileShare + "\" + $uploadFolder) -Persist -PSProvider "FileSystem" -Credential $userCredentials -Scope Global
...
# Map ASE Download Folder to t drive 
    try {
        New-PSDrive -Name "t" -Root ("\\" + $hostFileShare + "\" + $downloadFolder) -Persist -PSProvider "FileSystem" -Credential $userCredentials -Scope Global
...
# Get name of most recently edited subfolder witin the download folder and update project file to use
    ...
    ("download\" + $downloadFolderList[1].Name) | Out-File ("C:\inetpub\wwwroot\" + $projectFile)
